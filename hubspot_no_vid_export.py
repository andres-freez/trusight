...
OUTPUT_CSV = os.path.join(OUTPUT_DIR, f"{OUTPUT_BASENAME}_{TIMESTAMP}.csv")
...
def hubspot_get(
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            wait = 2 ** attempt
            log.warning("Timeout on %s. Retrying in %ds... (%s)", url, wait, e)
            time.sleep(wait)
            continue

        except requests.exceptions.ConnectionError as e:
            wait = 2 ** attempt
            log.warning("Connection error on %s. Retrying in %ds... (%s)", url, wait, e)
            time.sleep(wait)
            continue
    ...
    log.error("GET failed after %d retries: %s", MAX_RETRIES, url)
    return None
...
def build_row_from_result(
    ...
    for prop in history_props:
        ...
        current_raw_value = properties.get(prop, "")
        fallback_raw_value = row.get(prop, current_raw_value)
        history_value = selected_raw_value if selected_raw_value not in (None, "") else fallback_raw_value

        row[f"{prop}_history_value"] = map_option_label(
            prop,
            history_value,
            property_label_maps,
        )
